...
class SignalDetector:
    ...
    def detect_entry(previous_row: pd.Series, current_row: pd.Series) -> str:
        ...
        if pd.notna(previous_row["Slow SMA"]) and pd.notna(previous_row["Fast SMA"]):
            if (previous_row["Fast SMA"] <= previous_row["Slow SMA"]) and (current_row["Fast SMA"] > current_row["Slow SMA"]):
                return "Entry"
        return None
    ...
    def detect_exit(previous_row: pd.Series, current_row: pd.Series) -> bool:
        ...
        """
        return (previous_row["Fast SMA"] >= previous_row["Slow SMA"]) and (current_row["Fast SMA"] < current_row["Slow SMA"])
...
class CapitalManagement:
    ...
    def add_capital(self, amount: float):
        """Adds a given amount to the total capital.

        Args:
            amount (float): Amount of capital to be added.
        """
        self.liquid_capital += amount
